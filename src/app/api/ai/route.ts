...
IMPORTANT: Use color tags to indicate severity:
- [[RED]] for DANGEROUS/CRITICAL interactions
- [[YELLOW]] for WARNING/CAUTION interactions
- [[GREEN]] for safe/non-significant findings

Format example:
[[RED]] This combination is dangerous: ... [[END]]
[[YELLOW]] Be cautious with: ... [[END]]
[[GREEN]] These are compatible: ... [[END]]
...
Provide:
1. Dangerous interactions (use [[RED]])
2. Severity (Low/Medium/High)
3. Recommendations
Use color tags for each finding.`
...
IMPORTANT: Use color tags to indicate severity:
- [[RED]] for serious side effects or dangers
- [[YELLOW]] for warnings and precautions
- [[GREEN]] for general safe information
...
IMPORTANT: Use color tags for severity:
- [[RED]] for critical concerns/interactions
- [[YELLOW]] for cautions and warnings
- [[GREEN]] for safe findings

Analyze prescriptions for interactions, dosages, safety. Always add disclaimer.`

      userMessage = `Analyze this prescription: ${message}
Provide:
1. Summary of medicines (use [[GREEN]])
2. Potential concerns (use [[RED]] or [[YELLOW]])
3. Recommendations
Use color tags to highlight severity.`

    } else {
      systemPrompt = `You are MedCare AI assistant for a healthcare platform. Medicine database:
${JSON.stringify(medicineDatabase)}

IMPORTANT: Use color tags for clarity:
- [[RED]] for dangerous information
- [[YELLOW]] for important warnings
- [[GREEN]] for safe/positive information
